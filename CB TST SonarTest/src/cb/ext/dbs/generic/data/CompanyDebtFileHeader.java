...
public class CompanyDebtFileHeader {
	...
	public CompanyDebtFileHeader() {
		
	}
	
	public CompanyDebtFileHeader( long companyOID) {
		setFileDate(CBSystem.getInstance().getProcessDate().toDBString());
		setFileCurrency(CurrencyUtility.DEFAULT_CURRENCY);
	...
	public static CompanyDebtFileHeader get(long oid) throws CBException {
		...
		try {
			...
			if (pom.readByPrimaryKey(oid)) {
				...
				debtFileHeader.setDistrictCode(data.districtCode);
	...
	public long add() throws CBException{
		...
		try {
			...
			data.districtCode       = getDistrictCode();
