...
public class DBSCompanyResultFileSchedulePOMData extends POMData {
	protected DBSCompanyResultFileSchedulePOMData() {
	}
	...
	static final POMDataFieldInfo fieldInfo = new POMDataFieldInfo(new short[] {
			16, 1, 15, 16, 6, 1 }, new short[] { 0, 0, 0, 0, 0, 0 },
			new short[] { 0, 0, 0, 0, 0, 0 }, new short[] { 0, 0, 0, 0, 0, 0 },
			new String[] { "oID", "status", "lastUpdated", "companyOid",
					"time", "useLimit" }, new short[] { 0, 0, 0, 0, 0, 0 },
			new short[] { 0, 0, 0, 0, 0, 0 });
	private static final long iD = 3000012870000019L;
	...
	public boolean isChanged(Integer... arBagKeys) {
		DBSCompanyResultFileSchedulePOMData pomOrigin = (DBSCompanyResultFileSchedulePOMData) POMDataCache
	...
	public void copy(POMData data) {
		DBSCompanyResultFileSchedulePOMData cPOMData = (DBSCompanyResultFileSchedulePOMData) data;
		cPOMData.oID = oID;
		cPOMData.status = status;
		cPOMData.lastUpdated = lastUpdated;
		cPOMData.companyOid = companyOid;
		cPOMData.time = time;
		cPOMData.useLimit = useLimit;
	}

	public Object clone() {
		DBSCompanyResultFileSchedulePOMData clonePOMData = DBSCompanyResultFileSchedulePOMData
				.newInstance();
		copy(clonePOMData);
		return clonePOMData;
	}

	public static DBSCompanyResultFileSchedulePOMData newInstance() {
		DBSCompanyResultFileSchedulePOMData pomData = (DBSCompanyResultFileSchedulePOMData) POMDataPool
				.getFreePOMData(iD);
		if (pomData == null) {
			pomData = new DBSCompanyResultFileSchedulePOMData();
			POMDataPool.setPOMDataAsInUse(pomData);
		}
		return pomData;
	}

	public static DBSCompanyResultFileSchedulePOMData newInstance(CBBag inBag)
			throws CBException {
		DBSCompanyResultFileSchedulePOMData pomData = newInstance();
		pomData.setBag(inBag);
		return pomData;
	}

	public static DBSCompanyResultFileSchedulePOMData newInstance(CBRow row)
			throws CBException {
		DBSCompanyResultFileSchedulePOMData pomData = newInstance();
		pomData.setBag(row);
		return pomData;
